...
def build_u2i_matrix(user_item_score_data_path,
                     item_name_data_path,
                     write_file=False):
    ...
    with open(item_name_data_path, encoding="ISO-8859-1") as f:
        for line in f:
            ...
            item_id = int(item_id)
            item_id_to_item_name[item_id] = item_name  # item_id: item_name
    ...
    with open(user_item_score_data_path, encoding="ISO-8859-1") as f:
        for line in f:
            ...
            if user_id not in user_to_rating:
                user_to_rating[user_id] = [
                    0
                ] * total_movie_count  # user_id: [0, 0, ..., 0], value为list,长度为电影总数
    ...
    header = ["user_id"] + [
        item_id_to_item_name[i + 1] for i in range(total_movie_count)
    ]
    sheet.append(header)
    for i in range(len(user_to_rating)):
        #每行：user_id, rate1, rate2...
        line = [i + 1] + user_to_rating[i + 1]  # user_id从1开始
        sheet.append(line)
...
def cosine_distance(vector1, vector2):
    ab = vector1.dot(vector2)  #向量点乘
    a_norm = np.sqrt(np.sum(np.square(vector1)))  #向量模长，平方-求和-开方
...
def find_similar_item(user_to_rating):
    item_to_vector = {}
    total_user_count = len(user_to_rating)
    for user, user_rating in user_to_rating.items():  # int, list
        for moive_id, score in enumerate(user_rating):
            moive_id += 1  # 电影id从1开始
            if moive_id not in item_to_vector:
                item_to_vector[moive_id] = [0] * (total_user_count)
            item_to_vector[moive_id][
                user -
                1] = score  # # movie_id: [0, 0, ..., 0]，value为list,长度为用户总数
    #item_to_vector记录了每个用户打分，数据结构和user_to_rating一样
    #复用一下下方的相似度计算方法
    # 返回{item_id: [(item_id, similarity), (item_id, similarity),...]...} 相似度从大到小
    ...
    for user_a, ratings_a in user_to_rating.items():
        ...
        for user_b, ratings_b in user_to_rating.items():
            ...
            #相似度计算采取cos距离
                ...
                similarity = cosine_distance(np.array(ratings_a),
                                             np.array(ratings_b))
        ...
        user_to_similar_user[
            user_a] = similar_user  # user_id: [(user_id, similarity), (user_id, similarity),...] 相似度从大到小
...
def item_cf(user_id, item_id, item_to_similar_item, user_to_rating, topn=10):
    pred_score = 0
    count = 0
    for similar_item, similarity in item_to_similar_item[item_id][:topn]:
        # 用户对相似物品的打分
        rating_to_similar_item = user_to_rating[user_id][similar_item - 1]
        #分数*物品相似度，作为一种对分数的加权，越相似的物品评分越重要
        pred_score += rating_to_similar_item * similarity
        #如果这个相似物品用户没看过，就不计算在总数内
        if rating_to_similar_item != 0:
            count += 1
    pred_score /= count + 1e-5  # 防止分母为0
    return pred_score
...
def movie_recommand(user_id,
    ...
    #当前用户还没看过的所有电影id
    unseen_items = [
        item_id + 1 for item_id, rating in enumerate(user_to_rating[user_id])
        if rating == 0
    ]
    res = []
...
if __name__ == "__main__":
    user_item_score_data_path = "ml-100k/u.data"  # user-item-score-timestamp
    item_name_data_path = "ml-100k/u.item"  # item_id|item_name|...
    user_to_rating, item_to_name = build_u2i_matrix(user_item_score_data_path,
                                                    item_name_data_path, False)
    ...
    while True:
        user_id = int(input("输入用户id："))
        recommands = movie_recommand(user_id, similar_user, similar_items,
                                     user_to_rating, item_to_name)
