...
class DailogueSystem:
    ...
    def load_scenario(self, scenario_dir):
        # 默认初始记忆为空,加载时把available_node为true的node加入available_nodes
        for scenario_file in os.listdir(scenario_dir):
            if not scenario_file.endswith('.json'):
                continue
            with open(os.path.join(scenario_dir, scenario_file), "r", encoding="utf-8") as f:
                scenario = json.load(f)
                scenario_name = os.path.splitext(scenario_file)[0]
                for node in scenario:
                    if node.get("available_node") and node.get("available_node") == 'true':
                        self.available_nodes.append(scenario_name + node["id"])
...
if __name__ == '__main__':
    ds = DailogueSystem()
    print(ds.slot_to_qv)
    memory = {"available_nodes":ds.available_nodes}  #默认初始记忆为空
    while True:
