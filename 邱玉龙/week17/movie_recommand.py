...
def item_cf(user_id, item_id, similar_items, user_to_rating,topn=10):
    pred_score = 0
    count = 0
    # key 用户id 对应的相似电影id的打分，其中index索引为电影id
    # user_to_rating {1:[0,5,4,3]}
    # 电影 1 与其他电影的相似度
    #similar_items =》 {1:[[2, 1.55], [3,2.88]]}
    for similar_item, similarity in similar_items[item_id]:
        # 相似物品的评分
        # 用户 1 下喜欢的电影打分数值
        for rating_by_similar_item in user_to_rating[user_id]:
