...
def item_cf(user_id, item_id, similar_items, user_to_rating, topn=10):
    ...
    sum_scores = 0.0
    sum_similarity = 0.0
    for sim_item, similarity in similar_list[:topn]:
        # 用户对相似物品的评分，注意user_rating列表索引是item_id-1
        rating = user_to_rating[user_id][sim_item - 1]
        if rating != 0:  # 只考虑用户实际评分的物品
            sum_scores += similarity * rating
            sum_similarity += abs(similarity)
    # 避免除以零，若无相似物品则返回0
    if sum_similarity == 0:
        return 0.0
    return sum_scores / sum_similarity
