...
def item_cf(user_id, item_id, similar_items, user_to_rating, topn=10):
    similar_list = similar_items.get(item_id, [])
    total_score = 0.0
    total_similarity = 0.0

    for similar_item, similarity in similar_list[:topn]:
        # 获取用户对相似物品的历史评分（注意item_id从1开始）
        user_rating = user_to_rating[user_id][similar_item - 1]

        if user_rating != 0:
            total_score += user_rating * similarity
            total_similarity += similarity

    return total_score / (total_similarity + 1e-5)
...
def movie_recommand(user_id, similar_user, similar_items, user_to_rating, item_to_name, topn=10):
    ...
    for item_id in unseen_items:
        #user_cf打分
        score = item_cf(user_id, item_id, similar_user, user_to_rating)
        # item_cf打分
