...
def find_similar_item(user_to_rating):
    ...
    #复用一下下方的相似度计算方法
    return find_similar_user(item_to_vector, maxsize = 300)

#依照user对item的打分判断user之间的相似度
def find_similar_user(user_to_rating, maxsize = 100):
    ...
    for user_a, ratings_a in user_to_rating.items():
        ...
        for user_b, ratings_b in user_to_rating.items():
            #全算比较慢，省去一部分用户
            if user_b == user_a or user_b > maxsize or user_a > maxsize:
                continue
            #ab用户互换不用重新计算cos
...
#所以实现打分函数即可
#topn为考虑多少相似的用户，从用户中找到最相近的10个用户，用用户对作品的打分平均作为得分
#取前topn相似用户对该电影的打分
...
#作品与作品之间的相似度前文已经计算了，find_similar_item
#从作品中找到用户最喜欢的topk个作品，用户对喜欢作品的评分*喜欢作品和当前作品的相似度,因此要计算任意两个作品之间的相似度
def item_cf(user_id, item_id, similar_items, user_to_rating, topk=10):
    pred_score = 0
    count = 0
    #将用户的打分按从高到低排序
    sorted_items = sorted(enumerate(user_to_rating[user_id]), key=lambda x: x[1], reverse=True)
    # 提取前个元素(用户最喜欢的topk个作品)的索引
    indexs = [index for index, value in sorted_items[:topk]]
    # 预处理：提取作品item_id的相似度列表
    item_similarities = similar_items[item_id]

    for topk_id in indexs:
        similarity = next((score for id_, score in item_similarities if id_ == topk_id), None)
        # 用户对喜欢的作品的分数*作品相似度，
        if similarity == None:
            similarity = 0
        pred_score += user_to_rating[user_id][topk_id] * similarity
