...
def item_cf(user_id, item_id, similar_items, user_to_rating, topn=10):
    # 获取目标物品的相似物品列表
    similar_item_list = similar_items.get(item_id, [])
    # 用户已评分的物品及其评分
    user_rated = [(idx + 1, rating) for idx, rating in enumerate(user_to_rating[user_id]) if rating != 0]
    user_rated_dict = dict(user_rated)
    
    total_score = 0.0
    total_sim = 0.0
    count = 0
    # 遍历相似物品列表，取用户评分过的前topn个
    for sim_item, similarity in similar_item_list:
        if sim_item in user_rated_dict:
            rating = user_rated_dict[sim_item]
    ...
    if total_sim == 0:
        return 0.0  # 避免除以零
    pred_score = total_score / total_sim
    return pred_score
