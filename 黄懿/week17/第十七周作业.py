...
def movie_recommend(user_id, similar_user, similar_items, user_to_rating, item_to_name, topn=10):
    ...
    for item_id in unseen_items:
        # user_cf打分
        score_user_cf = user_cf(user_id, item_id, similar_user, user_to_rating)
        res_user_cf.append([item_to_name[item_id], score_user_cf])
        # item_cf打分
        score_item_cf = item_cf(user_id, item_id, similar_items, user_to_rating)
        res_item_cf.append([item_to_name[item_id], score_item_cf])
    # 排序输出
    res_user_cf = sorted(res_user_cf, key=lambda x: x[1], reverse=True)
    res_item_cf = sorted(res_item_cf, key=lambda x: x[1], reverse=True)
    return res_user_cf[:topn], res_item_cf[:topn]
...
if __name__ == "__main__":
    ...
    while True:
        user_id = int(input("输入用户id："))
        recommends_user_cf, recommends_item_cf = movie_recommend(user_id, similar_user, similar_items, user_to_rating, item_to_name)
        print("user_cf推荐：")
        for recommend, score in recommends_user_cf:
            print("%.4f\t%s" % (score, recommend))
        print("item_cf推荐：")
        for recommend, score in recommends_item_cf:
            print("%.4f\t%s" % (score, recommend))
