...
def item_cf(user_id, item_id, similar_items, user_to_rating, topn=10):
    ...
    pred_score = 0
    count = 0
    for similar_item, similarity in similar_items[item_id][:topn]:  # 不需要选择太多
        for item_id_in_similar_item, similarity in similar_items[similar_item][:topn]:
            if item_id_in_similar_item == item_id:
                continue
            pred_score += user_to_rating[user_id][item_id_in_similar_item - 1] * similarity
            count += 1
    pred_score /= count + 1e-5
    return pred_score
