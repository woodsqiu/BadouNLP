...
def build_u2i_matrix(user_item_score_data_path, item_name_data_path, write_file=False):
    #获取item id到电影名的对应关系
    item_id_to_item_name = {}  # {电影序号:电影名}
    with open(item_name_data_path, encoding="ISO-8859-1") as f:
        for line in f:
            item_id, item_name = line.split("|")[:2]  # [电影序号  电影名]
            item_id = int(item_id)
    ...
    #读打分文件
    user_to_rating = {}   # {用户:[所有电影,有打分的为分数,否则为0]}
    ...
    # print("user_to_rating:",user_to_rating)        
...
def item_cf(user_id, item_id, similar_items, user_to_rating):
    ...
    count = 0    
    # 用户所有喜欢的物品集
    nser_like_items = [item_like_id + 1 for item_like_id, rating in enumerate(user_to_rating[user_id]) if rating != 0]
    # 遍历喜欢的物品集
    for like_item in nser_like_items:
        # 遍历喜欢物品对其他物品的相似度
        for similar_item,similarity in similar_items[like_item]:
            rating_by_similiar_item = None
            if similar_item == item_id:
                rating_by_similiar_item = sum([value[item_id-1] for value in user_to_rating.values()])
                pred_score += rating_by_similiar_item * similarity
            if len([value for value in user_to_rating.values() if value != 0]) !=0:
                count +=1
    pred_score /= count + 1e-5   
    return pred_score
...
if __name__ == "__main__":
    user_item_score_data_path = r"D:\Python\PythonAi\pythonAi\week17\yuxi17\ml-100k\u.data"  # (用户 电影序号 分数 时间)
    item_name_data_path = r"D:\Python\PythonAi\pythonAi\week17\yuxi17\ml-100k\u.item"   # (电影序号|电影名|上映日期|路径)
    user_to_rating, item_to_name = build_u2i_matrix(user_item_score_data_path, item_name_data_path, False)
    # {用户:[所有电影,有打分的为分数,否则为0]}  {电影序号:电影名}
    #user-cf
