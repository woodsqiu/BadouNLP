...
def item_cf(user_id, item_id, similar_items, user_to_rating, topn=10):
    # 获取与该电影相似的电影及相似度
    similar_items_with_similarity = similar_items[item_id]
    # 得到的结果是 [[item_id, similarity], ...]
    def get_similarity(like_item_id):
        for id, similarity in similar_items_with_similarity:
            if id == like_item_id:
                return similarity
        return 0
    ...
    # 获取用户最喜欢的topn部电影
    like_items = sorted(enumerate(user_ratings), key=lambda x: x[1], reverse=True)[:topn]
    # 如果用户没有喜欢的电影，则返回0
    if len(like_items) == 0: return 0
    
    pred_score = 0
    for like_item_id, rating in like_items:
        # 计算相似度
        similarity = get_similarity(like_item_id)
        # 计算加权平均分
        pred_score += rating * similarity


    return pred_score / len(like_items)    
